...
LedStripe.prototype = {
  ...
  animate: function (name, buffer, config) {
    ...
    if (anim === 'rainbow') {
      intervalId = rainbow(buffer, config.speed);
    } else if (anim === 'flashing') {
      intervalId = flash(buffer, config.speed, config.colour);
    } else if (anim === 'kinghtrider') {
      intervalId = knightRider(buffer, config.speed);
    } else {
      intervalId = standard(buffer, config.speed, config.colour);
    }
  ...
  test: function (start, end) {
    if (!start) {
      ...
      setTimeout(function () {
        this.fill(0x00, 0xFF, 0x00);
      }, 1000);
      setTimeout(function () {
        this.fill(0x00, 0x00, 0xFF);
      }, 2000);
      setTimeout(function () {
        this.fill(0xFF, 0xFF, 0xFF);
      }, 3000);
      setTimeout(function () {
        this.disconnect();
      }, 4000);
