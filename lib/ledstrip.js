...
LedStripe.prototype = {
  ...
  connect: function (sectionStart, sectionEnd, spiDevice, gamma) {
    ...
    try {
      this.spiFd = fs.openSync(this.spiDevice, 'rs+');
      this.isOpen = true;
    } catch (err) {
  ...
  completeBuffer: function (origBuffer) {
    ...
    var newBuff = this.fill(0x00, 0x00, 0x00, true);
    newBuff = new Buffer(newBuff.length);
    fs.readSync(this.spiFd, newBuff, 0, newBuff.length, null);

    logger.log('info', 'Read from file. Buffer size is: %d', newBuff.length);
