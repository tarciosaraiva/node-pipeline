...
LedStripe.prototype = {
  ...
  connect: function (sectionStart, sectionEnd, spiDevice, gamma) {
    // reset some stuff
    this.gammatable = new Array(256);
    this.numLEDs = parseInt(conf.get('leds:length'));
    ...
    this.sectionStart = parseInt(sectionStart);
    this.sectionEnd = parseInt(sectionEnd);
  ...
  disconnectWithColour: function (colour) {
    if (this.spiFd) {
      var rgb = h2r(colour);
      var buff = this.fill(rgb.r, rgb.g, rgb.b);
      buff = this.completeBuffer(buff);
      this.sendRgbBuf(buff);
      fs.closeSync(this.spiFd);
      this.isOpen = false;
      this.spiFd = null;
  ...
  fill: function (r, g, b) {
    if (this.spiFd) {
      var i = 0,
        bufSize = (this.sectionEnd - this.sectionStart) * this.bytePerPixel,
